...
def find_best_two_layer_net(data):
    best_model = None
    best_val = 0
    best_solver = None
    best_params = {}
    ...
    num_epochs = 15
    for i in range(0, 10):
        lr, reg, weight_scale = generate_random_hyperparams(-5, -1, -3, -2, -7, -2)
        net_model = TwoLayerNet(input_size, hidden_size, num_classes, weight_scale=weight_scale, reg=reg)
    ...
    best_model.test_accuracy = np.mean(y_test_pred == data['y_val'])
