...
class Predictor_v2(object):
    ...
    def evaluate(self, sess, dataset: Dataset):
        ...
        for batch_idx in range(0, dataset.batch_count()):
            ...
            step += 1
            X, Y = dataset.next_batch(batch_idx)
            acc = sess.run(
                self._accuracy_op,
    ...
    def test_batch(self, sess, test_X, test_Y, unit_pixel, size, scale, save_path=''):
        def draw_bbx(ax, x, y):
