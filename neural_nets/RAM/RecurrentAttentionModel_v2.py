...
from neural_nets.RAM import ram_config_v2 as ram_config

# tf.disable_v2_behavior()


class RecurrentAttentionModel_v2(object):
    def __init__(self, config: ram_config):
        ...
        self.ORIG_W = config.ORIG_W
        self.ORIG_H = config.ORIG_H
    ...
    def glimpseSensor(self, img, normLoc):
        ...
        img = tf.reshape(img, (self.batch_size, self.img_W, self.img_H, self.channels))
    ...
    def evaluate(self, dataset: Dataset, sess, reward, predicted_labels, correct_labels, glimpse_images, draw=False):
        ...
        for batch_idx in range(batches_in_epoch):
            ...
            if draw:
                ...
                self.plotWholeImg(nextX[0, :], self.img_W, self.img_H, sampled_locs_fetched)
    ...
    def toMnistCoordinates(self, coordinate_tanh):
        ...
        '''
        x = np.round(((coordinate_tanh[:, :, 0] + 1) / 2.0) * self.img_W)
        y = np.round(((coordinate_tanh[:, :, 1] + 1) / 2.0) * self.img_H)
        return np.stack([x, y], axis=2)

    def plotWholeImg(self, img, W, H, sampled_locs_fetched):
        plt.imshow(np.reshape(img, [W, H]),
        ...
        with tf.device('/cpu:1'):
            with tf.Graph().as_default():
                ...
                self.inputs_placeholder = tf.placeholder(tf.float32,
                                                         shape=(self.batch_size, self.img_W * self.img_H),
                                                         name="images")
                ...
                self.Wr_h_r = self.weight_variable((self.cell_out_size, self.img_W * self.img_H),
                                                   "reconstructionNet_wts_hidden_action", True)
                self.Br_h_r = self.weight_variable((1, self.img_W * self.img_H), "reconstructionNet_bias_hidden_action",
                                                   True)
                ...
                for epoch in range(0, self.max_epochs):
                    ...
                    if iter % (iterations_per_epoch * 5) == 0:
