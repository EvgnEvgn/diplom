...
def get_glimpse(loc):
    ...
    glimpseFeature1 = tf.nn.relu(
        tf.matmul(act_glimpse_hidden, Wg_hg_gf1) + tf.matmul(act_loc_hidden, Wg_hl_gf1) + Bg_hlhg_gf1)
    # return g
    ...
    for t in range(nGlimpses):
        ...
        with tf.variable_scope("coreNetwork", reuse=REUSE):
            ...
            hiddenState = tf.nn.relu(
                affineTransform(hiddenState_prev, cell_size) + (tf.matmul(glimpse, Wc_g_h) + Bc_g_h))
...
def evaluate():
    data = dataset.test
    batches_in_epoch = len(data._data) // batch_size
    accuracy = 0
    ...
    for i in range(batches_in_epoch):
        ...
        r = sess.train(reward, feed_dict=feed_dict)
        accuracy += r
...
def convertTranslated(images, initImgSize, finalImgSize):
    ...
    for k in range(batch_size):
        ...
        # padding
        image = np.lib.pad(image, ((randX, size_diff - randX), (randY, size_diff - randY)), 'constant',
                           constant_values=(0))
        ...
        if eval_only:
            ...
            if preTraining:
                for epoch_r in range(1, preTraining_epoch):
                    ...
                    reconstructionCost_fetched, reconstruction_fetched, train_op_r_fetched = sess.train(fetches_r,
                                                                                                        feed_dict={
            ...
            for epoch in range(start_step + 1, max_iters):
                ...
                results = sess.train(fetches, feed_dict=feed_dict)
                ...
                if epoch % 20 == 0:
                    ...
                    summary_str = sess.train(summary_op, feed_dict=feed_dict)
                        ...
                        if animate:
                            ...
                            for y in range(nGlimpses):
                                txt.set_text('Epoch: %.6d \nPrediction: %i -- Truth: %i\nStep: %i/%i'
                                             % (epoch, prediction_labels_fetched[0], correct_labels_fetched[0], (y + 1),
                                                nGlimpses))
